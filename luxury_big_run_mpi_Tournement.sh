#!/bin/bash
#SBATCH --job-name=PARP_luxury_Tournement_Run_0
#SBATCH --output=/bgfs/jdurrant/jspiegel/Luxury_Run/Tournement_Lux/PARP_luxury_Tournement_Run_0.conf.out
...
highest_folder="/bgfs/jdurrant/jspiegel/Luxury_Run/Tournement_Lux/"
mkdir $highest_folder

# run precache
mpirun -n 1 ~/miniconda3/envs/py37/bin/python -m mpi4py /bgfs/jdurrant/jspiegel/autogrow/RunAutogrow.py -c
...
do
    ...
    echo "START Autogrow 4.0 Run number $i  STABILITY RUN USING with Tournement_QVINA2_5"
    start_time="$(date +%s%N | cut -b1-13)"
    date +%s%N | cut -b1-13

    mpirun -n $SLURM_NTASKS \
    ~/miniconda3/envs/py37/bin/python -m mpi4py /bgfs/jdurrant/jspiegel/autogrow/RunAutogrow.py \
        --filename_of_receptor /bgfs/jdurrant/jspiegel/autogrow/tutorial/PARP/4r6e_removed_smallmol_aligned_Hs.pdb \
        --center_x -70.76 --center_y  21.82 --center_z 28.33 \
        --size_x 25.0 --size_y 16.0 --size_z 25.0 \
        --source_compound_file /bgfs/jdurrant/jspiegel/autogrow/source_compounds/PARP_docked_ranked_ZINC_fragments.smi \
