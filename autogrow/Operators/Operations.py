...
def populate_generation(vars, generation_num):
    ...
    if generation_num == 1:
        ...
        # This will be done later but we are unpacking vars here
        num_to_advance_from_previous_gen = vars['number_to_advance_from_previous_gen_first_generation']
    else:
        # Later generations
        num_crossovers = vars['number_of_crossovers']
        num_mutations = vars['number_of_mutants']
        num_to_advance_from_previous_gen = vars['number_to_advance_from_previous_gen']


    # Get the Source compound list. This list is the full population from either the previous generations or
    # if its Generation 1 than the its the entire User specified Source compound list
    # If either has a SMILES that does not sanitize in RDKit it will be excluded and a printout of its Name and SMILES string
    # will be printed.
    ...
    total_num_desired_new_ligands = num_crossovers + num_mutations + num_to_advance_from_previous_gen
    ...
    save_ligand_list(vars['output_directory'], generation_num, new_mutation_smiles_list, "Chosen_Mutants")
    
    if new_mutation_smiles_list==None or len(new_mutation_smiles_list) < num_mutations:
        print("")
        print("")
        print("We needed to make {} ligands through Mutation".format(num_mutations))
        print("We only made {} ligands through Mutation".format(len(new_mutation_smiles_list)))
        print("")
        print("")
    ...
    save_ligand_list(vars['output_directory'], generation_num, new_crossover_smiles_list, "Chosen_Crossovers")
    

    if new_crossover_smiles_list==None or len(new_crossover_smiles_list) < num_crossovers:
        print("")
        print("")
        print("We needed to make {} ligands through Crossover".format(num_crossovers))
        print("We only made {} ligands through Crossover".format(len(new_crossover_smiles_list)))
        print("")
        print("")
    ...
    # This handles creating a seed list and defining the advance to next generation final selection

    chosen_mol_to_pass_through_list = make_pass_through_list(vars, source_compounds_list, num_to_advance_from_previous_gen, generation_num)

    if type(chosen_mol_to_pass_through_list) == str:
        printout = chosen_mol_to_pass_through_list+"\nIf this is the 1st generation, it may be due to the starting library has SMILES which could not be converted to sanitizable RDKit Molecules"
    ...
    # make a list of all the ligands from mutations, crossovers, and from the last generation
    ...
    if vars['redock_advance_from_previous_gen'] == False and generation_num != 1:
        for i in chosen_mol_to_pass_through_list:
            # Doesn't append to the new_generation_smiles_list
            full_generation_smiles_list.append(i)
    
    # Generation 0 pass through gets added to the convert and dock list because it has no docking score to compare with
    #  This is independent of the vars['redock_advance_from_previous_gen']
    else:
        for i in chosen_mol_to_pass_through_list:
            new_generation_smiles_list.append(i)
            full_generation_smiles_list.append(i)
    ...
    if len(full_generation_smiles_list) < total_num_desired_new_ligands:    
        ...
        full_generation_smiles_file, new_gen_folder_path = save_generation_smi(vars['output_directory'], generation_num, full_generation_smiles_list, None)

        # Save the File to convert to 3d
        smiles_to_convert_file, new_gen_folder_path = save_generation_smi(vars['output_directory'], generation_num, new_generation_smiles_list, "_to_convert")
...
# 
#############
def populate_generation_0(vars, generation_num=0):
    """
    ...
    # will be printed.
    source_compounds_list = get_complete_list_prev_gen_or_source_compounds(vars, generation_num)
    num_to_advance_from_previous_gen = len(source_compounds_list)
    ...
    # Make a list of the ligands chosen to pass through to the next generation
    # This handles creating a seed list and defining the advance to next generation final selection
    chosen_mol_to_pass_through_list = make_pass_through_list(vars, source_compounds_list, 1, 0)
    ...
    try:    
        ...
        full_generation_smiles_list_printout = ["\t".join(x) for x in full_generation_smiles_list]
        already_docked = True
    except:
        print("Not all ligands in source compound list are scored. We will convert and redock them all.")
    ...
    # If you are to redock and convert the generation zero you will also need to do the following:
    else:
        # CONVERT SMILES TO .sdf USING GYPSUM and convert .sdf to .pdb with rdkit
        # This will output sdf files into a folder. The .smi.0.sdf file is not a valid mol, but all the others will be valid
        # the 1st Smiles in the original .smi file is saved as .smi.1.sdf and 2nd file is saved as .smi.2.sdf
...
def test_source_smiles_convert(smile_info):
    """
    This attempts to convert a SMILES string to an rdkit.Chem.rdchem.Mol object
        -done in a try statement so that it is some bad SMILES string which is incapable
         of being converted
    ...
    if smile_info is None or len(smile_info)==0:
        printout = "REMOVING SMILES FROM SOURCE LIST: Blank entry in source compound list.\n"
    ...
    # This will fail if there are valence errors. We won't try to correct someones source compound list
    # Although the MOH.check_sanitization will do that.
    ...
    # Make the mol again fresh and try running it through MOH.handleHs()
    ...
    if mol is None:
        ...
        printout = printout + "\t This is often an issue with valence and sanitization issues with the SMILES string."
        printout = printout + "\t Removed SMILE string is: {} \n".format(smile_str)
        printout = printout + "\t Removed SMILE ID is: {}".format(smile_id)
        return printout
...
def get_complete_list_prev_gen_or_source_compounds(vars, generation_num):
    """
    Get the source compounds list from the previous generation of the source compound list
    
    This also filters the list to ensure mols can be imported to RDKit and that they pass the drug-likliness filters.
    
    If generation = 1 use the User specified starting compounds
    ...
    """
    source_file_gen_0 = vars['output_directory'] + "generation_{}{}generation_{}_ranked.smi".format(0, os.sep, 0)
    if generation_num == 0:
        ...
        if len(usable_list_of_smiles) == 0:
            ...
            raise Exception("There were no available ligands in source compound. Check formating")
        ...
        source_file = vars['output_directory'] + "generation_{}{}generation_{}_ranked.smi".format(generation_num-1, os.sep, generation_num-1)
    ...
    # Test that every SMILES in the usable_list_of_smiles is a valid SMILES which will import and Sanitize in RDKit.
...
def make_seed_list(vars, source_compounds_list, generation_num, num_seed_diversity, num_seed_dock_fitness):
    """
    Get the starting compound list for running the Mutation and Crossovers
    
    If generation = 0 use the User specified starting compounds
    ...
    if generation_num == 0:
        ...
        if vars["use_docked_source_compounds"] == False:
            ...
            if os.path.exists(source_file_gen_0) == False:
                ...
                # generation_num 1 may run into problems if the source compounds are smaller than the seed pool required
                # to seed generation 1. Because the seeding options are connected to the generation number 
                #       (due to the depreciation of diversity option)
                # Because of this we may need to ignore the ranking for the seeds of generation 1 to accomidate the smaller source size.
                #       This is especially important with lead-optimization in which the source pool may be much smaller
                # For this reason we will override the seeding of generation 1 if the number to seed is greater than exists but will provide a warning message.
                if len(usable_list_of_smiles) < num_seed_diversity or len(usable_list_of_smiles) < num_seed_diversity:
...
def determine_seed_population_sizes(vars, generation_num):
    ...
    """
    # How many fewer seed mols are chosen from diversity compared to the 1st generation
    # This is also how many more get chosen from elitist selection
    diversity_depreciation = int(generation_num - 1) * vars["diversity_seed_depreciation_per_gen"]

    if generation_num == 1:
        top_mols_to_seed_next_generation = vars["top_mols_to_seed_next_generation_first_generation"]
    ...
    num_seed_diversity = vars["diversity_mols_to_seed_first_generation"] - diversity_depreciation

    # Number of mols chosen because of their docking score. Elitist style selection 
    # but this will be chosen using a weight roulette selector later on.
    if num_seed_diversity <= 0:
        num_seed_dock_fitness = top_mols_to_seed_next_generation + vars["diversity_mols_to_seed_first_generation"]
        num_seed_diversity = 0
    elif num_seed_diversity > 0:
        num_seed_dock_fitness = top_mols_to_seed_next_generation + diversity_depreciation

    return num_seed_diversity, num_seed_dock_fitness
#

def make_pass_through_list(vars, smiles_from_previous_gen_list, num_to_advance_from_previous_gen, generation_num):
    """
    This function determines the molecules which will advance from the previous generation
    ...
    :param list smiles_from_previous_gen_list: List of SMILES from the last generation chosen to seed the 
                list of molecules to advance to the next generation without modification.
    :param int num_to_advance_from_previous_gen: the number of molecules to advance from the last
                generation without modifications.
    :param int generation_num: the interger of the current generation
    
    Return:
    :returns: list list_of_ligands_to_advance: a list of ligands which should advance into the new generation
                        without modifications from the last generation. Returns a printout of why it failed if it fails
    """
    ...
    if generation_num!=0 and len(smiles_from_previous_gen_list) < num_to_advance_from_previous_gen:
    ...
    if generation_num == 0 and has_dock_score == False:  
        # Take the 1st num_to_advance_from_previous_gen number of molecules from ligands_which_passed_filters
        ...
        list_of_ligands_to_advance = make_seed_list(vars, ligands_which_passed_filters, generation_num, len(ligands_which_passed_filters), num_to_advance_from_previous_gen)

    elif generation_num != 0 and has_dock_score == False:  
        # Take the 1st num_to_advance_from_previous_gen number of molecules from ligands_which_passed_filters
        random.shuffle(ligands_which_passed_filters)
        list_of_ligands_to_advance = []
        for x in range(0, num_to_advance_from_previous_gen):
            selected_mol = ligands_which_passed_filters[x]
            list_of_ligands_to_advance.append(selected_mol)
        ...
        # Use the make_seed_list function to select the list to advance. 
        # This list will be chosen strictly by 
        list_of_ligands_to_advance = make_seed_list(vars, ligands_which_passed_filters, generation_num, 0, num_to_advance_from_previous_gen)
        

    if generation_num == 0:
        return list_of_ligands_to_advance
    elif len(list_of_ligands_to_advance) >= num_to_advance_from_previous_gen:
        return list_of_ligands_to_advance
    else:
...
#############
def save_generation_smi(output_directory, generation_num, formatted_smile_list, nominclature_tag):
    """"
    This function saves a list of newly generated population of ligands as an .smi file.
    ...
    :param str nominclature_tag: The str describing the ligand list
                            if None than don't add tag. It is the full list of all ligands for the generation
    ...
    new_gen_folder_path = output_directory + "generation_{}{}".format(generation_num,os.sep)
    if nominclature_tag is None:
        # make the name for the new generation file
        output_file_name = new_gen_folder_path + "generation_{}.smi".format(generation_num)
    else:
        # make the name for the new generation file
...
def save_ligand_list(output_directory, generation_num, list_of_chosen_ligands, nominclature_tag):
    """
    Save the list of ligands.
    nominclature_tag is a string such as "Mutation" or "Crossover" or "Previous_Gen_choice" 
    ...
    :param str nominclature_tag: The str describing the ligand list
                            -ie seeding_mutations is the list that seeded the mutations
                                while mutations would be the list of mutations generated from the seeding_mutations list
    ...
    output_file_name = "{}{}_Gen_{}.smi".format(seed_folder_path, nominclature_tag, generation_num)
