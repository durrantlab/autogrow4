"""#NIH Filter 
This will filter a ligand using the NIH filter to remove 
ligands with undersirable functional groups.

This script relies on the RDKit predefined FilterCatalog.
    -FilterCatalog is maintained by RDKit.

If using the NIH filter please cite:
...
class NIHFilter(ParentFilter):
    ...
    The NIH filter is used to eliminate ligands with undersirable functional groups
    ...
    def run_filter(self, mol):
        """
        Runs a NIH filter by matching common false positive molecules to
        the current mol. 
