...
This script relies on the RDKit predefined FilterCatalog.
    -FilterCatalog is maintained by RDKit.

If using the PAINS filter please cite:
Baell JB, Holloway GA. New Substructure Filters for Removal 
of Pan Assay Interference Compounds (PAINS) from Screening Libraries 
and for Their Exclusion in Bioassays. 
J Med Chem 53 (2010) 2719D40. doi:10.1021/jm901137j.
"""
import __future__
...
class PAINS_Filter(ParentFilter):
    ...
    def get_filters_list(self):
        ...
        This loads in the filters which will be used.
        Returns:
        :returns: rdkit.Chem.rdfiltercatalog.FilterCatalog filters: A set of RDKit Filters
        """
        # Make a list of all the different PAINS Filters
        # PAINS should include PAINS_A,PAINS_B, and PAINS_C, but because
        # RDKit documentation doesn't specify this explicitly we have included
        # all 4 of the PAINS FilterCatalogs for precaution.
    ...
    def run_filter(self, mol):
        """
        Runs a PAINS filter by matching common false positive molecules to
        the current mol. 
        ...
        for filters in self.filters_list:
            ...
            #If the mol matches a mol in the filter list
            #we return a False (as it failed the filter)
            if filters.HasMatch(mol) is True:
                return False
            else:
                continue

        #if No matches are found to filter list
        #this will return a True as it Passed the filter.
        return True
    #
#
