...
from autogrow.Operators.Crossover.SmilesMerge.MergeFunctions import Alignment_and_Breaks as ANB
from autogrow.Operators.Crossover.SmilesMerge.MergeFunctions import Dict_and_R_Groups as DRG
from autogrow.Operators.Crossover.SmilesMerge.MergeFunctions import MappingClass as MC
from autogrow.Operators.Crossover.SmilesMerge.MergeFunctions import Merge_w_core as MWC
...
class CrossoverTests(unittest.TestCase):
    ...
    def test_Mapping_locate_Bs(self):
        ...
        self.assertEqual(aMapping.locate_Bs(10000),['1B1'])

        for i in self.Is_to_Bs.keys():
            items = self.Is_to_Bs[i]
             
            self.assertEqual(aMapping.locate_Bs(i),items)
    def test_Mapping_locate_Is(self):
        aMapping = copy.deepcopy(self.aMapping) 
        
        self.assertEqual(aMapping.locate_Is('1B1'),[10000])
        
        for i in self.Bs_to_Is.keys():
            items = self.Bs_to_Is[i]
            self.assertEqual(aMapping.locate_Is(i),items)
    def test_Mapping_delete_B(self):
        aMapping = copy.deepcopy(self.aMapping) 
        self.assertEqual(self.aMapping.locate_Bs(10002), ['2B2','2B3'])
        self.assertEqual(aMapping.locate_Bs(10002),['2B2','2B3'])

        aMapping.delete_B('2B2')

        self.assertNotEqual(aMapping.locate_Bs(10002),['2B2','2B3'])
        self.assertNotEqual(aMapping.locate_Bs(10002), self.aMapping.locate_Bs(10002))
        self.assertEqual(aMapping.locate_Bs(10002),['2B3'])
    def test_Mapping_delete_I(self):
        aMapping = copy.deepcopy(self.aMapping) 
        self.assertEqual(self.aMapping.locate_Is('2B3'), [10003, 10002])
        self.assertEqual(aMapping.locate_Is('2B3'), [10003, 10002])

        aMapping.delete_I(10002)

        self.assertNotEqual(aMapping.locate_Is('2B3'), [10003, 10002])
        self.assertNotEqual(aMapping.locate_Is('2B3'), self.aMapping.locate_Is('2B3'))
        self.assertEqual(aMapping.locate_Is('2B3'), [10003])
    ...
    def test_unpack_lists_of_atoms_and_bond_type(self):
        ...
        CoreMerg_iso_to_Idx_dict = {10000: 0, 10001: 1, 10002: 2, 10003: 3, 2004: 4, 2005: 5, 2006: 6, 2007: 7, 2008: 8, 2009: 9, 2010: 10, 2011: 11, 2012: 12, 2013: 13}

        list_of_atom_idx, list_of_bond_types = MWC.unpack_lists_of_atoms_and_bond_type(anchor_to_connection_dict, anchor_atom_iso, CoreMerg_iso_to_Idx_dict)
        ...
        CoreMerg_iso_to_Idx_dict2 ={1000: 0, 1001: 1, 1002: 2, 1004: 4, 1005: 5, 1006: 6, 1007: 7, 1008: 8, 1009: 9, 10003: 3}
        list_of_atom_idx2, list_of_bond_types2 = MWC.unpack_lists_of_atoms_and_bond_type(anchor_to_connection_dict2, anchor_atom_iso2, CoreMerg_iso_to_Idx_dict2)
    ...
    def test_unpack_lists_of_atoms_and_bond_type(self):
        ...
        ###########anchor_to_connection_dict, anchor_atom_iso, CoreMerg_iso_to_Idx_dict)
        ###########
    ...
    def test_map_all_neighbors_w_iso(self):
        ...
        neighbor_dict = DRG.map_all_neighbors_w_iso(mol)
        known_dic= {1000: [1001], 1001: [1000, 1002], 1002: [1001, 1003], 1003: [1002, 1004], 1004: [1003, 1005], 1005: [1004, 1006], 1006: [1005, 10007], 10007: [1006]}

        self.assertEqual(neighbor_dict,known_dic)

        # Same mol as above but as ligand 2 instead of ligand 1
        #
        # this should be a dic with 8 entries, only 10000 and 100007 should have 1 entry, all else have 2
        smile = "[2000CH3][2001CH2][2002CH2][2003CH2][2004CH2][2005CH2][2006CH2][10007CH3]"
        mol = Chem.MolFromSmiles(smile)
        neighbor_dict = DRG.map_all_neighbors_w_iso(mol)
        known_dic= {2000: [2001], 2001: [2000, 2002], 2002: [2001, 2003], 2003: [2002, 2004], 2004: [2003, 2005], 2005: [2004, 2006], 2006: [2005, 10007], 10007: [2006]}
