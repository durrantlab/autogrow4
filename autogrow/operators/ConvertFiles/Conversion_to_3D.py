...
class StdoutRedirection:
    ...
    Class taken from https://stackoverflow.com/questions/4110891/how-to-redirect-the-output-of-print-to-a-txt-file
    """
...
def convert_to_3d(vars, smi_file, smile_file_directory):
    """
    This function converts SMILES from 1D to 3D using gypsum 
...
def convert_smi_to_sdfs_with_gypsum(vars, gen_smiles_file, smile_file_directory):    
    """
    Convert a file of SMILES to a set of 3d .sdf files using Gypsum. 
    This does so by making a set of .json files for running Gypsum for every ligand in the .smi file.  

    This will print out the list of ligands which failed to convert to 3D.
    
    Inputs:
    :param dict vars: User variables which will govern how the programs runs
    :param str gen_smiles_file: the file name of the .smi file to be converted to 3D sdf's
    :param srt smile_file_directory: the directory path which contains the .smi file     
    Returns:
    :returns: str gypsum_output_folder_path: a path to the folder with all of the 3D sdf's created by gypsum.
    """    
    ...
    # Make a new folder to put gypsum .smi's and json
    # Name folder gypsum_submission_files
    ...
    list_of_jsons = make_smifile_and_gyspum_submitfile(gen_smiles_file, folder_path, gypsum_output_folder_path,max_variance, gypsum_thoroughness, min_ph, max_ph, pka_precision)
...
def make_smifile_and_gyspum_submitfile(gen_smiles_file, folder_path, gypsum_output_folder_path, max_variance, gypsum_thoroughness, min_ph, max_ph, pka_precision):
    """
    Make an individual .json file to submit to Gypsum for every ligand in the .smi file. 
    It then executes gypsum for every .json file. This also makes a .smi file for each ligand which will
    be required for Gypsum.

    The .smi file for each ligand will be noted within the .json file to instruct Gypsum where to find the SMILE.
       
    Inputs:
    :param str gen_smiles_file: the file name of the .smi file to be converted to 3D sdf's
    :param srt folder_path: the directory path which will contain the inputs and outputs from Gypsum     
    :param str gypsum_output_folder_path: a path to the folder with all of the 3D sdf's created by gypsum.
    :param int max_variance: User variable for how many conformers per ligand should be made by Gypsum
    :param int gypsum_thoroughness: User variable for How widely Gypsum-DL will search for low-energy conformers.
                        Larger values increase run times but can produce better results 
    :param float min_ph: User variable for Minimum pH to consider by Dimorphite-DL
    :param float max_ph: User variable for Maximum pH to consider by Dimorphite-DL
    :param float pka_precision: User variable for Size of pH substructure ranges by Dimorphite-DL
    Returns:
    :returns: list list_of_jsons: a list of the paths to all .jsons to be submitted to Gypsum.
    """    
    list_of_jsons = []
    ...
    with open(gen_smiles_file) as smiles_file:
        for line in smiles_file:
            ...
            if len(parts)==0 or len(parts)==1:
                print(parts)
            smile = parts[0]
            # ligand_name example       (Gen_30_Cross_639427+Gen_31_Cross_717928)Gen_34_Cross_709666
            # But bash doesn't like + or () for file names so we will abridge
            # lig_name_short name for above example becomes Gen_34_Cross_709666 
            # if ligand is from the source files we wont split the name
            ...
            if len(ligand_name.split(")"))==2:
                ...
                printout = "Ligand name failed to abridge. Smiles may be named in improper format \
                            ...
                            The failed ligand name was {}".format(ligand_name)
            ...
            gypsum_out_sdf_path = "{}{}.sdf".format(gypsum_output_folder_path, lig_name_short)
            ...
            "max_ph": {},
            "pka_precision": {}
            }}'''.format(smi_path, gypsum_output_folder_path, \
                max_variance, gypsum_thoroughness, min_ph, max_ph, pka_precision)
...
def run_gypsum_multiprocessing_mpi(gypsum_log_path, json_path, timeout_option, gypsum_timeout_limit, python_path):
    ...
    This uses a Try statement and a bash script to be able to create a timeout if it requires
    more than 5 minutes to convert a single ligands
    ...
    # The only information that is import is the json_path, which will be used to define all
    # the parameters
...
def run_gypsum_multiprocessing(gypsum_log_path, json_path, timeout_option, gypsum_timeout_limit, python_path):
    ...
    run_single_gypsum_executable  = "{}{}run_single_gypsum_{}.sh".format(current_dir_executables, os.sep, timeout_option)
    gypsum_dl_py_executable  = "{}{}gypsum_dl{}run_gypsum_dl.py".format(current_dir_executables, os.sep, os.sep)

    # run as a .sh bash script to be able to use timeout feature and pipe log
    # There is a timeout limit for gypsum defined by users defined within run_single_gypsum_executable_timeout
...
def check_gypsum_log_did_complete(log_file_path):
    """
    This function checks a log_file_path to see if the last line
    reads "TIMEOUT". If it does then gypsum timed out before converting
    a .smi. If it timedout return False. If it completed return True

    Inputs:
    :param str log_file_path: the path to the log from converting a ligand with Gypsum. 
    Returns:
    :returns: bool bol:  Returns True if the conversion worked (or if it was a blank .smi with no name or SMILES) 
                        Returns False if it timedout.
                        Returns None if it failed to convert due to errors with the SMILES string, .json, or .smi
                                -examples would be
                                        1) if the SMILE was invalid chemically
                                        2) There was SMILE name but no SMILE string
                                        3) The SMILES wasn't a SMILE but rather a different data type ie None, False, non-chemical string...
    """   
...
def convert_sdf_to_pdbs(vars, gen_folder_path, sdf_folder_path):
    ...
    # create a new subfolder if one doesn't already exist. 
    # folder will be with the generation and will be titled PDBs
    # pdb_subfolder_path will become the the output folder
