...
def main():
    ...
    # Root options
    parser = MolvsParser(
        epilog='use "molvs <command> -h" to show help for a specific command'
    )
    ...
    common_parser.add_argument(
        "-i", "--intype", help="input filetype", choices=FILETYPES
    )
    common_parser.add_argument(
        "-:", "--smiles", help="input SMILES instead of file", metavar="<smiles>"
    )
    common_parser.add_argument(
        "-O",
        "--outfile",
    ...
    standardize_parser = subparsers.add_parser(
        "standardize", help="standardize a molecule", parents=[common_parser]
    )
    standardize_parser.add_argument(
        "-o", "--outtype", help="output filetype", choices=FILETYPES
    )
    standardize_parser.set_defaults(func=standardize_main)

    # Validate options
    validate_parser = subparsers.add_parser(
        "validate", help="validate a molecule", parents=[common_parser]
    )
...
def _read_mol(args):
    if args.smiles:
        ...
        args.intype in {"smi", "smiles"}
        or args.infile.name.endswith("smi")
        or args.infile.name.endswith("smiles")
    ):
        return Chem.MolFromSmiles(args.infile.read())
    elif (
        args.intype in {"mol", "sdf"}
        or args.infile.name.endswith("mol")
        or args.infile.name.endswith("sdf")
    ):
...
def _write_mol(mol, args):
    if (
        args.outtype in {"smi", "smiles"}
        or args.outfile.name.endswith("smi")
        or args.outfile.name.endswith("smiles")
    ):
        args.outfile.write(Chem.MolToSmiles(mol))
        args.outfile.write("\n")
    elif (
        args.outtype in {"mol", "sdf"}
        or args.outfile.name.endswith("mol")
        or args.outfile.name.endswith("sdf")
    ):
