"""#BRENK Filter 
This will filter a ligand using the BRENK filter for lead-likeliness,
by matching common false positive molecules to the current mol..

This script relies on the RDKit predefined FilterCatalog.
    -FilterCatalog is maintained by RDKit.

If using the BRENK filter please cite:
Brenk R et al. 
...
class BRENKFilter(ParentFilter):
    ...
    If using the BRENK filter please cite:
    Brenk R et al. Lessons Learnt from Assembling Screening 
    Libraries for Drug Discovery for Neglected Diseases. 
    ChemMedChem 3 (2008) 435-444. doi:10.1002/cmdc.200700139.
    ...
    def run_filter(self, mol):
        """
        Runs a BRENK filter by matching common false positive molecules to
        the current mol. Filters for for lead-likeliness.
