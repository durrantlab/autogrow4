...
def handle_mcs_align_labeling_and_cyclicbreaks(mol_1, mol_2, mcs_mol):
    """
    This will:
        take 2 aligned molecules and pick a specific alignment, 
        check for any ring and cyclic breaks and fragmentation
            -removing any atoms from the common core which caused those issues
    ...
    if are_there_breaks is True:
        # Run cyclic ring break again, if there are still breaks create sys error and abort merge
        mcs_M3, new_index2, are_there_breaks_2 = check_cyclic_breaks(new_index, mol_1, mol_2, mcs_mol)
    ...
    # Add correct isolabels to the core atoms
    # requires 1 final fake alignment tuple of range(0,len(mcs_mol.GetNumAtoms()))
    ...
    # label the 1st atom in R-group with its idx + (1000 for lig1 and 2000 for lig 2)
    # the 1000 vs 2000 label will be used to deconvelute later
    # these will be the iso values assigned to the 1st atom in an R-group
...
########################## 
# USE THESE FUNCTION TO TEST FOR CYCLIC BREYCLE BREAK AND REMOVE BAD ATOMS FUNCTIONS               
##########################
...
def check_cyclic_breaks(alignment_tuple, mol_1, mol_2, core):
    ...
    Fixing cyclic breaks is handled by removing the atoms from the core (MCS substructure).
    Removing atoms will often cause fragmentation (ie. removing a carbon from a methyl
    will leave 3 fragmented hydrogens).
    Fragmented atoms need to be removed. The largest fragment is assumed to be the core of interest
    which we don't want to remove.

    Inputs:
    :param tuple alignment_tuple: a tuple with the atoms with IDX which match in the same order
                                ie. alignment_tuple[0][0] is the IDX for the atom in mol_1 which 
                                is matched to alignment_tuple[1][0] and alignment_tuple[2][0] (for mol_2)
                                alignment_tuple[3] (for core) is the reference index which ranges from 
                                0 to the number of atoms in the order (0,1,2,3,4...n)
    :param rdkit.Chem.rdchem.Mol mol_1: rdkit mol for ligand 1
    :param rdkit.Chem.rdchem.Mol mol_2: rdkit mol for ligand 2
    :param rdkit.Chem.rdchem.Mol core: rdkit mol for shared common core between mol_1 and mol_2

    Returns:
    :returns: rdkit.Chem.rdchem.Mol core: the original core rdkit mol returned if did_a_ring_break==False
    :returns: tuple alignment_tuple: the unaltered input param alignment_tuple returned if did_a_ring_break==False
    :returns: bool did_a_ring_break: True if the ring broke and was fixed; False if there were no breaks
                                    and required no modifications to be made to the alignment_tuple or core
    :returns: rdkit.Chem.rdchem.Mol new_core:           the modified core rdkit mol returned if did_a_ring_break==True
    :returns: tuple new_align_tuple: the modified alignment_tuple returned if did_a_ring_break==True
    ...
    if len(mcs_ringbreak_idx)>0:
        ...
        # THIS IS A BIT COMPLEX HERE SO THIS IS THE IDEA:
        # Using the list of ringbreak idx's we will delete those ringbreak atoms 
        # from the core. This will change the idx's within the core which is why we've 
        # iso-labeled the core in previous steps.
        # Once we delete from the core we need to detemine the idx's of fragmented atoms
        # but these will be the idx's in the original core, as determined by the isotope
        # labels.
        #  ie. idx_of_atom_in_original_core = atom.GetIsotope()-10000
        # (We keep the largest fragment as the future core). 
        # 
        # After we have a list of all the atom idx's (in the original core) for all the atoms
        # which are ringbreaks and all the atoms that will cause cyclicbreaks/fragmentation
        # we will then delete all those atoms at once, thus preventing issues of the idx's 
        # changing as we delete and issues of fragmentation.
        
        # Make a copy of the core to test fragments
        temp_core_removed_breaks = copy.deepcopy(core)

        # delete atoms causing ringbreaks
        temp_core_removed_breaks = MOH.remove_atoms(temp_core_removed_breaks, mcs_ringbreak_idx)
        if temp_core_removed_breaks is None:
            return None, None, None
        # check for fragmentation and add the smallest fragments indexes to the to delete list
        all_atoms_to_delete = ringbreak_frag_handling(temp_core_removed_breaks, mcs_ringbreak_idx)
        if all_atoms_to_delete is None:
            return None, None, None

        # Now work on the original core
        # THIS WILL BE THE OFFICIAL NEW CORE
        ...
        # now that we've made a new core, the idx's are different so we need to relabel
        # mol_1 and mol_2
        ...
        # this is done using the original indexing and the atoms which were removed
        # from mcs
        count = 0
        for l1,l2,c1 in zip(alignment_tuple[0],alignment_tuple[1],alignment_tuple[2]):
...
def ringbreak_frag_handling(new_core, mcs_ringbreak_idx):
    """
    This takes a rdkit mol of the core (after atoms were removed to resolve cyclic breaks).
    It then tests and handles ringbreaks and fragmentation.
    
    if an atom in the core was deleted, if it had additional atoms attached to it it 
    ...
    """
    ########################## 
    # Check for fragmentation, if core is fragmented than additional processing is required to find the largest frag
    # and to then reassign the indexes in the ligs and core
    ...
    if num_frag_len > 1:
        ...
        # these will be the idx's of the original common core, before deleting things
        # which will be identified using the Isolabels we added before
        # We will be deleting these atoms shortly
        for frag in list_frag_mols:

            # get all atom idx's (for the original unaltered common_core)
            # in the frag based on the Iso-labels we added before
...
def pick_mcs_alignment(mol_1, mol_2, common_core):
    """
    This will take the common substructure (aka the common_core) and find every match to it within
    each of the two ligands and produce to list of tuples for the atom index (Idx) relative 
    to the indexing of the common_core substrure.
    
    It will then randomly pick combination of an alignment for mol_1 and mol_2.
    
    This should pick the alignments for future numbering.
   
    This should return:
        picked_alignment (tuple)    picked_alignment[0] is alignment for mol_1
                                    picked_alignment[1] is alignment for mol_2
                                    picked_alignment[2] is the atom idx of the common_core
            ie. picked_alignment[0][0] is the atom IDx for the atom in mol_1 which corresponds
                        to the atom in the common substructure index as 0
                picked_alignment[0][1] is the atom IDx for the atom in mol_1 which corresponds
                        to the atom in the common substructure index as 1
                picked_alignment[1][0] is the atom IDx for the atom in mol_2 which corresponds
                        to the atom in the common substructure index as 0
                picked_alignment[1][1] is the atom IDx for the atom in mol_2 which corresponds
                        to the atom in the common substructure index as 1
    ...
    Returns:
    :returns: tuple picked_alignment: tuple with 3 sub lists of the atoms IDx which correspond to their respective
                                mol object in the order that they match the atom in the same index in the sublist
                                for all 3 mols (mol_1,mol_2,common_core)
    """
    # Get the substructure match for the MCS within each ligand
    mol_1_match_idx = mol_1.GetSubstructMatches(common_core, uniquify = False, maxMatches=10)
    mol_2_match_idx = mol_2.GetSubstructMatches(common_core, uniquify = False, maxMatches=10)
    ...
    # Check that it is a list of tuples. 
    #otherwise the random.choice function breaks in python 2.7
...
def add_mcs_isolabels(mol_1, mol_2, common_core, picked_alignment):
    ...
    :param tupple picked_alignment: a tuple with 3 subtuples for mol_1,mol_2,common_core
                                    The numbers within the sublist is the atom IDx for a given atom in a ligand.
                                    The sublist index for each atom in for picked_alignment corresponds to the Idx
                                    of that atoms match in the commmon_core
                                    ie picked_alignment[1][3] = 10
    ...
    i = 0    
    index_list = []
    for lig1, lig2, c1 in zip(picked_alignment[0],picked_alignment[1],picked_alignment[2]):
        atom1 = mol_1.GetAtomWithIdx(lig1)
        atom2 = mol_2.GetAtomWithIdx(lig2)        
        atomC = common_core.GetAtomWithIdx(c1)
          
        atom1.SetIsotope(10000+i)
        atom2.SetIsotope(10000+i)
        atomC.SetIsotope(10000+i) 
        index_list.append(i)
        i = i+1
...
def renumber_to_mcs(mol, tuple_order_list):
    ...
    :param tuple tuple_order_list: a tuple with 3 subtuples for mol_1,mol_2,common_core
                                The numbers within the sublist is the atom IDx for a given atom in a ligand.
                                The sublist index for each atom in for tuple_order_list corresponds to the Idx
                                of that atoms match in the commmon_core
                                ie tuple_order_list[1][3] = 10
                                    Thus the IDx atom in mol_2 which corresponds to the 3rd atom in the common_core
                                    is 10.
    :returns:                             
    :returns: rdkit.Chem.rdchem.Mol mol: the same rdkit molecule but with the atom idx's renumbered to be consistent with the
                                        common core, as provided by the tuple_order_list                              
    """
    # make a tuple the same length as the mol
    # the beggining needs to be the same as the aligment numbering to the MCS
    # and there can be no redundancy in the tuple.
