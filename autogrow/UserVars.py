...
def handle_Alternative_filters(vars, filter_list):
    ...
    if vars["alternative_filter"]!= None:
        ...
        if len(scripts_to_copy)!=0:
            ...
            print(" AutoGrow needs to be restarted and should now be able to run custom scripts.")
            print("Please ensure you unit test this code properly before incorprating.")
            print("#############################################################################\n")
            sys.exit(0) # Technically Exit intentionally but maybe should be a raise Exception
    return filter_list
#
    ...
    if vars["custom_scoring_script"]!= None:
        ...
        if Custom_class[0] not in full_children_dict.keys():
            ...
            if os.path.exists(new_file)==True: 
                ...
                printout = "\n#############################################################################"
                printout = printout + "AutoGrow has incorporated the custom files into the Scoring Module."
                printout = printout + "AutoGrow needs to be restarted and should now be able to run custom scripts."
                printout = printout + "Please ensure you unit test this code properly before incorprating."
                printout = printout + "#############################################################################\n"
