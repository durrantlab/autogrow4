...
*   **AutoGrow4 Libraries**: To enrich the initial set, the pipeline also
    ...
    13, 2019. We thank ZINC for allowing us to distribute these fragment
...
*   **Stereoisomer Generation**: For molecules in still-underpopulated
    ...
    stereochemistry. These new, unique molecules are added if they pass all
    quality filters.

This augmentation loop continues until all moiety libraries meet the target
count or until no new valid molecules can be generated. In practice, it is not
possible to generate more than 5,000 molecules for every moiety while adhering
to the strict filtering criteria. Each moiety library is saved to a gzipped
SMILES file.
