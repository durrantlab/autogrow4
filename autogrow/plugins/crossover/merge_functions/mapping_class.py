...
class Mapping(object):
    """
    Handles mapping of B-groups to anchor atoms for molecular merging.

    This class manages the selection of B-groups (branching groups) that will
    be used to create a child molecule from two parent ligands. It handles
    complex cases where B-groups may connect to multiple anchor atoms.

    Attributes:
        b_to_is (Dict[str, List[int]]): Maps B-groups to anchor atom IDs.
        i_to_bs (Dict[int, List[str]]): Maps anchor atom IDs to B-groups.
    ...
    Notes on terminology:
        ...
        - Node/anchor/I: An atom in the MCS which has 1 or more atom(s)
          connected to it which are not part of the MCS
            - The anchors are labeled by their Isotope numbers as those do not
              get modified, where as atom Idx are modified by many Rdkit
              functions. Anchors have Isotope labels of 10,000 or higher and
              that label is applied to the MCS and ligands 1 and 2 so
              everything is trackable.
        - R-group: A chain of one or more atoms connected to a single node
            - If an anchor has dimethyl's which are not part of MCS then each
              methyl is considered its own R-group
        - B-group: 1 or more R-groups which branch off a single node.
            - If an anchor has dimethyl's which are not part of MCS then the
              combination of both methyls is considered a single B-group.

    B-group Naming scheme: '{first_number}B{second_number}'
        - first_number: The number before the B corresponds to the parent
          ligand from which the B-group is derived.
        - second_number: The number which follows the B is the order for which
          that B-group was determined when condensing the R-groups into
          B-groups. Numbering is indexed to 1. So the 1st three B groups for
          parent ligand 1 are: 1B1,1B2,1B3
        
    Examples:
        1B1 is the 1st B-group from parent ligand 1
    ...
    Example:
        If 1B1 connects to anchor atom 10003 and 10004; and 2B1 connects to
    ...
    def __init__(self, b_to_is: Dict[str, List[int]], i_to_bs: Dict[int, List[str]]):
        """
        Initialize the Mapping object with B-group and anchor mappings.

        When a Mapping object is initialized, it imports 2 input dictionaries,
        which can be referenced throughout the class.

        Args:
            b_to_is (Dict[str, List[int]]): Dictionary converting B-groups to
                anchor/node/I atoms. This contains groups from both parent
                molecules. This is the inverse of i_to_bs. Keys are B-groups;
                items are the anchor atoms isotope label.
                Example: {'1B1': [10003], '2B3': [10005], '2B2': [10003],
                '2B1': [10002]}
            i_to_bs (Dict[int, List[str]]): Dictionary converting
                Anchor/node/I atoms to corresponding B-groups. This contains
                groups from both parent molecules. This is the inverse of
                b_to_is. Keys are the anchor atoms isotope labels; items are
                B-groups.
                Example: {10002: ['2B1'], 10003: ['1B1', '2B2'], 10005: ['2B3']}
        """
    ...
    def _locate_b(self, i: int) -> List[str]:
        """
        Find all B-groups bound to a given anchor atom.

        Given a specified anchor/I return a list of all the B-groups from both
        parent ligands, bound to that anchor

        Args:
            i (int): The isolabel of an anchor atom which will be used to
                search for B-groups within self.i_to_bs.

        Returns:
            List[str]: A list of all the B-groups, from both parent ligands
                which are bound to that anchor. Example: ['1B1','2B1']
        """
        return self.i_to_bs[i]

    #

    def _locate_i(self, b: str) -> List[int]:
        """
        Find the anchor atom(s) a given B-group connects to.

        Given a specified B-group return the anchor/I/node it connects to.

        Args:
            b (str): The name of a B-groups within self.b_to_is.

        Returns:
            List[int]: A list of the anchor(s) the given B-groups is bound to.
                Example: [10001]
        """
    ...
    def _delete_b(self, b: str) -> None:
        """
        Remove a B-group from all mappings.

        Removes the b from b_to_is and all references to b in i_to_bs.
        b is a Key in b_to_is. B is one or more items in i_to_bs.

        Args:
            b (str): A B-group to be removed from the b_to_is and B in i_to_bs
                dicts.
        """
        i_list_to_modify = self._locate_i(b)
    ...
    def _delete_i(self, i: int) -> None:
        """
        Remove an anchor atom from all mappings.
    ...
    def _chose_b_from_i(self, i: int) -> Union[str, None]:
        ...
        choosing that B-group forces the future decision because of its
        ...
        Example (if 1B1 is connected to multiple anchors):
        Lack of an B-groups bound to an anchor is not considered a B-group

        Args:
            i (int): An integer representing the isolabel for an anchor/node/i
                atom. This function choses which B-group will be bound to this
                anchor in the child molecule.

        Returns:
            str: A string of the name of a chosen B-group.
            None: If not in the dictionary or if there is no available choices.
        ...
        Notes:
            ...
            1) Dominant Nulls: (Null is an option which cannot be override)
             ...
             appropriate to add Null.
               Example: if parent lig_1 has 1B1 attached to anchors 10000,10001,10002
            ...
            2) Recessive Nulls: (A Null can be chosen but it can be overriden)
             ...
             most code and computational efficient method, with no distinct
        ...
        list_is = self._locate_i(b_x)
        list_bs = [self._locate_b(x) for x in list_is]
...
def run_mapping(
    ...
    Args:
        b_dict (Dict[str, List[int]]): Dictionary converting B-groups to
            anchor/node/I atoms. This contains groups from both parent
            molecules. This is the inverse of i_to_bs. Keys are B-groups; items
            are the anchor atoms isotope label.
            Example: {'1B1': [10003], '2B3': [10005], '2B2': [10003],
            '2B1': [10002]}
        i_dict (Dict[int, List[str]]): Dictionary converting Anchor/node/I
            atoms to corresponding B-groups. This contains groups from both
            parent molecules. This is the inverse of b_to_is. Keys are the
            anchor atoms isotope labels; items are B-groups.
            Example: {10002: ['2B1'], 10003: ['1B1', '2B2'], 10005: ['2B3']}

    Returns:
        List[str]: A list of all the chosen B-groups to be used to generate a
            child molecule later.
    """
    a_mapping_object = Mapping(b_dict, i_dict)
    bs_chosen = []
    for i in i_dict:
        b_choice = a_mapping_object._chose_b_from_i(i)
