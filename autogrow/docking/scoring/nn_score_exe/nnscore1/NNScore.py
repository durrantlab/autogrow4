    ...
    def file_to_double_array(self, filename):
        ...
        for line in lines:
            ...
            if line[:6] == "CONEC:":
                ...
                conec.append(pair)
            if line[:5] == "INNO:":
                inno.append(int(line[5:]))
            if line[:6] == "OUTNO:":
                outno.append(int(line[6:]))
            if line[:4] == "ENI:":
                pair = line[4:]
                pair = pair.split(" ")
                pair[0] = float(pair[0])
...
class region:
    ...
    def point_in_region(self, point, padding):
        ...
        if self.region_type == "SPHERE":
            dist = math.sqrt(
                math.pow(point.x - self.center.x, 2)
                + math.pow(point.y - self.center.y, 2)
                + math.pow(point.z - self.center.z, 2)
            )
    ...
    def points_set(self, reso):
        ...
        if self.region_type == "BOX":
            ...
            # self.box_dimen
            for x in therange(
                self.center.x - self.box_dimen.x / 2,
                self.center.x + self.box_dimen.x / 2,
                reso,
            ):
                for y in therange(
                    self.center.y - self.box_dimen.y / 2,
                    self.center.y + self.box_dimen.y / 2,
                    reso,
                ):
                    for z in therange(
                        self.center.z - self.box_dimen.z / 2,
                        self.center.z + self.box_dimen.z / 2,
                        reso,
                    ):
            ...
            for x in therange(
                self.center.x - self.radius, self.center.x + self.radius, reso
            ):
                for y in therange(
                    self.center.y - self.radius, self.center.y + self.radius, reso
                ):
                    for z in therange(
                        self.center.z - self.radius, self.center.z + self.radius, reso
                    ):
                        thepoint = point(x, y, z)
                        thepoint.snap(reso)
                        if self.point_in_region(
                            thepoint, 0
                        ):  # padding always 0 for inclusion objects
                            points.append(thepoint)
        return points
...
class atom:
    ...
    def CreatePDBLine(self):
        ...
        output = output + self.element.rjust(
            24
        )  # + "   " + str(uniqueID) # This last part must be removed
        return output
    ...
    def LoadPDB_from_list(self, lines):
        ...
        for t in range(0, len(lines)):
            ...
            if len(line) >= 7:
                ...
                if (
                    line[0:5] == "ATOM " or line[0:7] == "HETATM "
                    ...
                    self.AllAtoms[
                        autoindex
                    ] = TempAtom  # because points files have no indecies
...
class Complex:
    def not_trained(self, type, key, ligand_atom, receptor_atom, ligand_filename):
        toreturn = (
            "The program can't deal with the "
            + type
            + " calculation between ligand,receptor atom types: "
            + key
            + ", ligand = "
            + ligand_filename
            + "\n"
        ...
        for ligand_index in ligand.AllAtoms:
            ...
            if lig_type not in self.lig_types:
                self.bad_training = (
                    self.bad_training
                    + "\n\n"
                    + "The program can't deal with ligands that have the atom type: "
                    + lig_type
                )
            else:
            ...
            for receptor_index in receptor.AllAtoms:
                ...
                if dist < 0.5:
                    print(
                        "There may be steric clashes between "
                        + ligand_name
                        + ", "
                        + receptor_name
                    )
                ...
                if dist < 4.0:
                    ...
                    if key in charge_type_combos:
                        ...
                        coulomb = (
                            lig_charge * recep_charge / dist
                        )  # ignore all constants. Just let the neural net take care of that
...
for index in range(1, len(sys.argv)):
    ...
    if var.upper() == "-RECEPTOR":
        receptor_name = sys.argv[index + 1]
    if var.upper() == "-LIGAND":
        ligand_name = sys.argv[index + 1]
    if var.upper() == "-VINA_OUTPUT":
        vina_output = sys.argv[index + 1]
    if var.upper() == "-AUTODOCK_OUTPUT":
        autodock_output = sys.argv[index + 1]  ###### ADD TO USER MANUAL #####
    if var.upper() == "-NETWORK":
        networks.append(sys.argv[index + 1])
    if var.upper() == "-NETWORKS_DIR":  # a directory containing only networks
        ...
        if sys.argv[index + 1][-1:] != "/":
            sys.argv[index + 1] = sys.argv[index + 1] + "/"
        for filename in os.listdir(sys.argv[index + 1]):
            if os.path.isfile(sys.argv[index + 1] + filename):
                networks.append(sys.argv[index + 1] + filename)
...
if error is True:
    ...
    print(
        "network files and specify the path to that directory using the -networks_dir"
    )
    print("parameter.")
    print("")
    ...
    print("")
    print(
        "Note: If an AutoDock Vina or AutoDock output file is specified, NNScore will"
    )
    print(
        "evaluate all docked poses and return the best NNScore calculated. To score an"
    )
    print(
        "AutoDock output file, modify the mglenv and prepare_ligand4_location variables"
    )
    print("at the begining of this python file.")
    print("")
    print("Examples:")
    print(
        "\t python NNScore.py -receptor neuraminidase.pdbqt -ligand oseltamivir.pdbqt -network ./networks/top_3_networks/12.net"
    )
    print(
        "\t python NNScore.py -receptor integrase.pdbqt -ligand raltegravir.pdbqt -networks_dir ./networks/top_3_networks/"
    )
    print(
        "\t python NNScore.py -receptor integrase.pdbqt -vina_output docked_poses.vina.out -networks_dir ./networks/top_3_networks/"
    )
    print(
        "\t python NNScore.py -receptor integrase.pdbqt -autodock_output docked_poses.autodock.out -networks_dir ./networks/top_3_networks/"
    )
    print(
        "\t python NNScore.py -receptor protease.pdbqt -ligand tipranavir.pdbqt -networks_dir ./networks/top_24_networks/ -network ./networks/top_3_networks/16.net"
    )
    print("")
    sys.exit()
...
def process_ligand(ligand_name, ligand, receptor):
    ...
    print("")
    print(
        "\tRelative coulombic energy between atom types (one ligand, one receptor) within 4 angtroms of each other:"
    )
    first = 1
    something = 1
    ...
    for line in lines:
        if "MODEL " in line:
            if len(thelines) != 0:
                ...
                average_score = process_ligand(
                    vina_output + ", " + label, ligand, receptor
                )
