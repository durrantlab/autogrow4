...
class NN2(VINA):
    ...
    def find_files_to_score(self, file_path):
        ...
        Return:
        :returns: list list_of_NN2_files: list of all files to be scored within the dir
        """   
    ...
    def run_rescoring(self, vina_output_file):
        """
        Run the NN2 scoring on all of these files.
        Return a list of a rescored file with NN2  
          ie *.pdbqt.vina.nn2        
        Inputs:
            ...
            while True:
                ...
                if "Best Score:" in line:
                    ...
                    try:
                        ...
                        continue
                    break
                elif "When the poses were ranked by the best of the 20 network scores" in line:
                    line = f.readline()
                    ...
                    try:
                        ...
                        continue
                    score = float(tmp[0])
                    # Make it a negative number since NN2 & NN2 a higher number is best
                    # But autogrow 4.0.0 uses the most negative number is best.
...
def run_nn_rescoring(vars, vina_output_file):
    ...
    Returns A list containing the file name as item 1 and whether it passed as item 2
    #     [PATH, True] means it passed
    #     [PATH, False] means it failed a results of all NN2 files
    ...
    torun = sys.executable + " "+ nn2_executable + " -receptor " + receptor + \
            " -ligand " + lig + " -vina_executable " + docking_executable + \
            " > " + str(nn2_output)
...
def execute_nn_scoring(command, file_path):
    ...
    returns None if it worked
    If it failes to rescore it returns the NN2 output file which failed to be produced.
