...
class VINA(ParentScoring):
    ...
    def run_rescoring(self, vina_output_file):
        """
        This is not applicable but is kept because the other rescoring functions require this function.
        Inputs:
        :param str vina_output_file: Path to a vina output file to be rescored
        Returns:
        :returns: str "Not Applicable": Because this doesn't need to rescore the docking results
        """
    ...
    def run_scoring(self, file_path):
        ...
        This is required for all Scoring Functions. Additional manipulations may go here but there are none for this script..
        ...
        :returns: list list_of_lig_data: information about the scored ligand. Score is last index
                    (ie. [lig_id_shortname, any_details, fitness_score_to_use] )
        """    
    ...
    def get_score_from_a_file(self, file_path):
        ...
        Returns:
        :returns: list lig_info: a list containing all info from self.smiles_dict for a given ligand
                    and the ligands short_id_name and the docking score from the best pose.
        """
    ...
    def merge_smile_info_w_affinity_info(self, lig_info):
        """
        From the info in self.smiles_dict get that info and merge that with the affinity info
        
        This will also replace the original SMILES string with that of the SMILES string in the PDB which conservers stereoChem
        
        Inputs:
        :param list lig_info: list containing [ligand_short_name, affinity]

        Returns:
        :returns: list ligand_full_info: a list containing all info from self.smiles_dict for a given ligand
                    and the ligands short_id_name and the docking score from the best pose.
                    returns None if ligand_short_name isn't in the self.smiles_dict which should never happen
        """
        ...
        if os.path.exists(pdb_path):
            ...
            if new_smiles_string==None:
                # If the REMARK SECTION IS NOT THERE raise except
                # Avoid this if possible as rdkit can missinterpret bonds because pdbs dont specify bond types
                # mol = Chem.MolFromPDBFile(pdb_path)
                # new_smiles_string = Chem.MolToSmiles(mol, isomericSmiles=True,canonical=True)
                raise Exception("Could not get SMILES string from PDB file: "+pdb_path)
        else:
            return None


        if ligand_short_name in list(self.smiles_dict.keys()):
            ligand_full_info = [new_smiles_string, self.smiles_dict[ligand_short_name][1]]
