...
def create_seed_list(usable_list_of_smiles, generation_num, num_seed_diversity, num_seed_dock_fitness, selector_choice, tourn_size):
    """
    this function will take ausable_list_of_smiles which can be derived from either the previous generation or a source_compounds_file. 
         Then it will select a set of smiles chosen by a weighted function to their docking-fitness (docking score) 
         Then it will select a set of smiles chosen by a weighted function to their diversity-fitness (docking score) 
        Then it will merge these sets of smiles into a single list

    Using the merged list it will make a list of all the smiles in the merged-list (chosen_mol_list) with all the other information
    about each of the chosen mols from the usable_list_of_smiles
    ...
    :param int selector_choice: the choice of selector method. Choices are Roulette_Selector, Rank_Selector, or Tournement_Selector
    :param float tourn_size: percentage of the total pool of ligands to be tested in each tournement.  
    Returns:
    :returns: list chosen_mol_full_data_list: a list of all the smiles in a weighted ranking
                ie ["CCCC"  "zinc123"   1    -0.1]
    """
    ...
    if selector_choice == 'Roulette_Selector':
        ...
        print('Rank_Selector')
        # This assumes the most negative number is the best option which is true for both 
        # This is true for both the diversity score and the docking score.
        ...
        raise Exception('selector_choice value is not Roulette_Selector, Rank_Selector, nor Tournement_Selector')
    ...
    if selector_choice == 'Rank_Selector' or selector_choice == 'Roulette_Selector':
        # Get all the information about the chosen molecules.
        # chosen_mol_list is 1D list of all chosen ligands
        # chosen_mol_full_data_list is a 1D list with each item of the list having multiple pieces of information
        #   such as the ligand name/id, the smiles string, the diversity and docking score...
        chosen_mol_full_data_list = get_chosen_mol_full_data_list(chosen_mol_list, usable_list_of_smiles)

    elif selector_choice == 'Tournement_Selector':
        # Tournement_Selector returns an already full list of ligands so you can skip the get_chosen_mol_full_data_list step
...
def get_chosen_mol_full_data_list(chosen_mol_list, usable_list_of_smiles):
    ...
    """
    This function will take a list of chosen molecules and a list of all the SMILES which could have been chosen and all
    of the information about those SMILES (ie. ligand name, SMILES string, docking score, diversity score...)

        It will iterated through the list of chosen mols (chosen_mol_list), get all the information from the usable_list_of_smiles
        Then it appends the corresponding item in usable_list_of_smiles to a new list weighted_order_list

            --- an issue to be aware of is that there may be redundancies in both chosen_mol_list and usable_list_of_smiles
                this causes a many-to-many problem so if manipulating this section you need to solve for one-to-many
                ---for this reason if this gets altered it will raise an AssertionError if the one-to-many is violated.

    It then shuffles the order of the list which to prevent biasing by the order of the ligands.
    ...
    Returns:
    :returns: list weighted_order_list: a list of all the SMILES with all of the associated information in a random order
    """
    ...
    if len(weighted_order_list) != len(chosen_mol_list):
        raise AssertionError("weighted_order_list not the same length as the chosen_mol_list")
...
def convert_usable_list_to_lig_dict(usable_list_of_smiles):
    ...
    :param list usable_list_of_smiles: list of SMILES and their associated information formated into a list
                                    which is usable by the rest of Autogrow
    Returns:
    :returns: list usable_dict_of_smiles: djct of all the ligand info with a key containing both the SMILE string 
                                    and the unique lig ID
    """
...
def score_and_append_diversity_scores(molecules_list):
    """
    This function will take list of molecules which makes up a population. 
    It will then create a diversity score for each molecules:
    It creates the diversity score by determining the Morgan Fingerprint for each molecule
        in the population. 
    It then compares the fingerprints for every molecule against every molecule in a pairwise manner.
        Based on the approach provided on http://www.rdkit.org/docs/GettingStartedInPython.html 
            section: "Picking Diverse Molecules Using Fingerprints"
        It determines a score of similarity using the RDKit function DataStructs.DiceSimilarity
            -The higher the similarity the higher the similarity score
                -ie) if you compare two identical SMILES the similarity score is 1.0. 
                        -ie) if you compare 4 identical SMILES the similarity score for each is 4.0. 
                -ie) if you compare two completely different SMILES, the score is 0.0
        ...
        It then appends the diversity score to the molecule list which it returns.

        It can raise an AssertionError if there are ligs which fail to sanitize or deprotanate.
                -this prevents future errors from occuring in later steps and makes this funciton usable
                    for multiple codes
        It will remove any Nones from the input list

    Inputs:
    :param list molecules_list: list of all molecules in the populations with the respective info
    Returns:
    :returns: list molecules_list: list of all molecules in the populations with the 
                                respective info and append diversity score
    """
    ...
    for pair in molecules_list:
        if pair is not None:
            ...
            if mol is None:
                raise AssertionError("mol in list failed to sanitize. Issue in Ranking.py \
    ...
    for i in range(0,len(fps_list)):
        ...
        for j in range(0,len(fps_list)):
            if i != j:
                # if DiceSimilarity=1.0 its a perfect match, the smaller the number 
                # the more diverse it is. The sum of all of these gives the distance
                # from the normal. The smaller the number means the more distant
                diversity_score = diversity_score + DataStructs.DiceSimilarity(fps_list[i][-1],fps_list[j][-1])
