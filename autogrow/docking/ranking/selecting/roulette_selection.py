...
def spin_roulette_selector(usable_list_of_smiles, number_to_chose, docking_or_diversity):
    """
    ...
    :param str docking_or_diversity: an string describing either "docking" or "diversity"
                                this tells the function how to adjust the weighted scores

    Returns:
    :returns: list top_choice_smile_order: list of ligands chosen by a random weighted selection, without replacement, 
                                        -weighted by its docking score
    """
    ...
    top_choice_smile_order = rn.choice(smiles_list, size=number_to_chose, replace=False, p=probability)

    return top_choice_smile_order

def adjust_scores(usable_list_of_smiles, docking_or_diversity):
    """
    This function adjusts the scores appropriately. This is where we weight the scores so smaller differences are more
    pronounced and where we adjust for the fact that docking score is better with a more negative number
    ...
    Returns:
    :returns: list adjusted: list of ligand scores which have been weighted and adjusted 
    """
    if docking_or_diversity == "docking":
        weight_scores = [float(x[-1]) for x in usable_list_of_smiles]
        # minimum is the most positive value from usable_list_of_smiles 
        # the more negative the docking score the better the dock
        ...
        # adjust by squaring the number to make the discrpency larger
        # and invert by dividing 1/x^2 (because the more diverse a mol is
