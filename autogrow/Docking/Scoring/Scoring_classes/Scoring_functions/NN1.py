...
class NN1(VINA):
    ...
    def run_rescoring(self, vina_output_file):
        """
        Run the NN1 scoring on all of these files.
        Return a list of a rescored file with NN1  
          ie *.pdbqt.vina.nn1
        ...
        with open(file_path, 'r') as f:
            for line in f.readlines():
                ...
                if "Best score: " in line:
                    ...
                    # Pose info
                    # not used here but could be useful in some context
                    # tmp = line.replace(")",'')
                    # tmp = tmp.split(', MODEL ')
...
def run_nn_rescoring(vars, vina_output_file):
    """
    This will run NN1 on all of the vina files in the list.
    ...
    networks_dir = os.path.dirname(vars['nn1_script']) + os.sep + "networks" + os.sep + "top_3_networks" + os.sep

    nn1_output = vina_output_file + ".nn1"

    torun = "python " + nn1_executable + " -receptor " + receptor + \
            " -vina_output " + vina_output_file + " -networks_dir " + networks_dir + \
            " > " + nn1_output
