...
def exist_generation_0(infolder: str) -> bool:
    ...
    for gen_folder in os.listdir(infolder):
        if os.path.isdir(os.path.join(infolder, gen_folder)):
            ...
            if "generation_0_" in gen_folder_name:
                return True
    return False
...
def make_graph(dictionary: Dict[str, Union[float, str]], analyze_gen_0: bool, exist_gen_0: bool) -> Tuple[Optional[List[int]], Optional[List[float]]]:
    """
...
def run_score_plotter(
    ...
    list_generations_average, list_of_scores_average = make_graph(average_affinity_dict, analyze_gen_0, exist_gen_0)

    print_top_15 = all(top_15[key] != "N/A" for key in top_15.keys())
    if print_top_15:
        list_generations_15, list_of_scores_15 = make_graph(top_15, analyze_gen_0, exist_gen_0)

    print_top_10 = all(top_10[key] != "N/A" for key in top_10.keys())
    if print_top_10:
        list_generations_10, list_of_scores_10 = make_graph(top_10, analyze_gen_0, exist_gen_0)

    print_top_5 = all(top_5[key] != "N/A" for key in top_5.keys())
    if print_top_5:
        list_generations_5, list_of_scores_5 = make_graph(top_5, analyze_gen_0, exist_gen_0)

    list_generations_one, list_of_scores_one = make_graph(top_1, analyze_gen_0, exist_gen_0)
    ...
    if print_top_5:
        ax.plot(list_generations_5, list_of_scores_5, color="g", label="Top 5")
...
def run_boxplot(
    ...
    exist_gen_0: bool = False
) -> None:
    data = []
    yticklabels = []
    for i in range(len(dictionary_of_values) - (1 if exist_gen_0 and not analyze_gen_0 else 0)):
